...
<title>Insert title here</title>
    <style>
        ...
        .ec-base-box.agreeAreaAll p{
            font-size: 14px;
            color: #666;
            line-height: 22px;
        }
        .ec-base-box #agree_all {
            width: 15px;
            height: 15px;
            margin: 0 0 0 13px;
        }
    ...
    <form action="/join" method="post" id="memberJoin">
    	...
    	<table class="join-div-table" style="width:100%;">
			...
			<tr>
				...
				<td><input type="password" name="pw" id="pw" class="form-control" pattern="(?=.*[a-zA-Z])(?=.*[!@#$%^*+=-])(?=.*[0-9]).{8,15}" required><span>(영문 대소문자/숫자/특수문자 포함한 8자~15자)</span></td>
			</tr>
			<tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="text" name="" id="addr_postcode"  placeholder="우편번호" readonly="readonly">
			...
			<tr>
				<th>생년월일</th>
				<td><input type="text" name="age" id="age" class="form-control" placeholder="ex)19970112"></td> 
			</tr>
            <tr>
                <th>성별</th>
                <td>
			...
			<tr>
				...
				<td><input type="text" name="email" id="email" class="form-control"> <button type="button" id="email_btn" class="btn-box" onclick="" value="이메일인증">이메일 인증</button><span id="emailChkMsg" required></span></td> <!--클릭하면 창뜨고 3분내로 입력하도록 시계뜨고 이메일로 인증번호 날려주고 그 값 입력하면 인증되었습니다 뜨게하기-->
        ...
        <div class="join_agree_wrap" style="width:100%;">
            ...
            <div class="join_agree">
                <div class="ec-base-box agreeAreaAll">
                    ...
                    <span for="agree_all" class="allCheck">이용약관 및 개인정보수집 및 이용,쇼핑정보 수신(선택)에 모두 동의합니다.</span>
                </div>
...
“슬로우앤드”는 회원가입, 주문안내, 배송안내 등 비광고성 정보를 카카오톡 알림톡으로 알려드리며, 만약 알림톡 수신이 불가능하거나 수신 차단하신 경우에는 일반 문자메시지로 보내드립니다. 카카오톡 알림톡을 통해 안내되는 정보를 와이파이가 아닌 이동통신망으로 이용할 경우, 알림톡 수신 중 데이터 요금이 발생할 수 있습니다. 카카오톡을 통해 발송되는 알림톡 수신을 원치 않으실 경우 반드시 알림톡을 차단하여 주시기 바랍니다.
                    ...
                    <p class="check">
                        ...
                        <input type="checkbox" id="agree_service_check" name="agree_service_check" value="1">
...
※ 동의를 거부할 수 있으나 거부시 회원 가입이 불가능합니다.</p>    
                    ...
                    <p class="check">
                        ...
                        <input type="checkbox" id="agree_privacy_check" name="agree_privacy_check" value="1">
...
//	1. 비밀번호 두개 안맞으면 비밀번호 다 리셋되고 커서이동
//	2. 아이디 바로바로 있는건지없는지 확인되게 (옆에 '사용중인 아이디입니다' 이런식으로 뜸)
//	3. 주소 우편번호만 빼고 세개 붙여서 디비에 저장되게
//	4. 이메일 양식 안맞으면 알림뜨게(pattern이용)
//	5. 이메일 인증누르면 인증번호치는 창뜨고(3분유효시간) 이메일로 인증번호 전송된거 입력하면 '인증되었습니다' 옆에 뜨게. 
//	6. 아이디/비밀번호 찾기 구현(창떠서 탭메뉴로)
